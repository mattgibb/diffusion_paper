...
DIFFUSION_PAPER_PATH=$( cd "$( dirname "$0" )/.." && pwd )
...
extract_slice() {
  ...
  $BINARY_PATH/SplitVolumeIntoSlices \
    ...
    $DIFFUSION_PAPER_PATH/$OUTPUT_DIR -e tiff -l -d $DIM -s $SLICE
}
...
flip_slice() {
  # build arguments
  TIFF_PATH=$DIFFUSION_PAPER_PATH/$1
...
convert_slice() {
  # build arguments
  TIFF_PATH=$DIFFUSION_PAPER_PATH/$1
  PDF_PATH=$DIFFUSION_PAPER_PATH/$2
