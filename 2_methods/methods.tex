...
\label{sec:methods}
  ...
  \begin{figure*}[!t]
    \centerline{\subfloat[]{\includegraphics[width=2.4in]{2_methods/Figs/absolute_errors_3d}
        \label{fig:absolute_errors_3d}}
        \hfil
        \subfloat[]{\includegraphics[width=2.4in]{2_methods/Figs/absolute_errors_2d}
        \label{fig:absolute_errors_2d}}
        \hfil
        \subfloat[]{\includegraphics[width=2.4in]{2_methods/Figs/relative_errors_2d}
        \label{fig:relative_errors_2d}}}
    \caption{(a) The 3D evolution of mean Euclidian dist blah blah. (b) The initial (blue), final (green) and sequentially registered (c) .
  ...
  Figure~\ref{fig:synthetic_errors}~\textbf{(a)} graphs the evolving mean Euclidean distance of the pixels within a tissue segmentation from their unperturbed ground truth position, after repeated applications of the smoothing algorithm. For almost all slices, the metric is greatly reduced over the 40 iterations, evidence that each slice is much more closely aligned to its true position. Most of the reduction is yielded in the earlier iterations, when the highest frequencies of noise are damped most easily.
  
  Figure~\ref{fig:synthetic_errors}~\textbf{(b)} graphs the initial (blue) and final (green) mean Euclidean distances of segmented pixels from their ground truth position (the starting and ending values from \textbf{(a)}), along with distances of the serially registered volume (red). Almost ubiquitously, the value is substantially lower after transformational diffusion, with the green lines appearing much smoother after the high frequency spectrum has been preferentially damped. DISCUSS RED LINE
% section methods (end)
